...
FEC::New(int rxlimit, int dataShards, int parityShards)  {
    if (dataShards <= 0 || parityShards <= 0) {
        //std::cout << dataShards << " parityShards: " << parityShards << std::endl;
    ...
    //std::cout << "0xffffffff/uint32_t(fec.totalShards) - 1)" << int(0xffffffff/uint32_t(fec.totalShards) - 1) << std::endl;
    return fec;
}
...
FEC::MarkFEC(byte *data) {
    ...
    //std::cout << "this->next: " << int(this->next) << " paws: " << int(this->paws) << std::endl;
}
...
FEC::Input(fecPacket &pkt) {
    ...
    //std::cout << "rx.size: " << this->rx.size() << " pkt->seqid: " << pkt.seqid << " n: " << int(n) << std::endl;
    int insertIdx = 0;
    for (int i=n;i>=0;i--) {
        //std::cout << "loop " << i << " rx[" << i << "].seqid=" \
        << rx[i].seqid << " pkt.seqid=" << pkt.seqid << std::endl;
    ...
    //std::cout << "rx.insert pkt: " << pkt.seqid << std::endl;
    rx.insert(rx.begin()+insertIdx, pkt);
    /*
     * std::cout << "rx.insert: " << insertIdx << " pkt.seqid: " << pkt.seqid << " pkt.data: " ;
